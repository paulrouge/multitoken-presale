...
class Config(metaclass=Singleton):
    ...
    def __init__(self, endpoint: str, keystore_path: str, passwd=None) -> None:
        ...
        if passwd is not None and endpoint in ('local', 'gochain'):
            self.accounts = []
            for i in range(3):
                wallet = KeyWallet.create()
                self.tx_handler.transfer(self.owner, wallet.get_address(), in_loop(200))
                self.accounts.append(wallet)
                # save the test keystore
                path = f'test_keystore{i}'
                if os.path.exists(path):
                    os.remove(path)
                wallet.store(path, 'gochain')
